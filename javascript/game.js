...
generateGameBoard()
    Generates the game board based off of the number of
    symbols passed in from the startButton event listener 

shuffleCards()
    Shuffles cards array passed in by setting random index of array
    to value of i in a for loop.

setupCards()
    Adds even listeners to all of the td's in gameTable.
    In the event listener, when clicked, the class value is set to 
    "flipped" as long as the class value is not already "matched"

checkMatch()
    Iterates through the td elements and checks the two cards that
    are currently flipped. If the cards that are flipped have the 
    same value (same symbol), then the cards are marked as a match
    and the class for each are set to "matched"

checkWin()
    Iterates through the td elements to check if all of the cards 
    have class value = "matched". If all of the class values are 
    "matched", the cards are removed and a <p> element is created 
    and displays a winning message to the user.
